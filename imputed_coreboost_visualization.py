...
# Load the imputed CoreBoost dataset
imputed_coreboost_data = pd.read_csv('data/imputed_coreboost.csv')

# Visualization 1: Fitness Goal vs. Average Weekly Consumption (Bottles)
plt.figure(figsize=(10, 6))
coreboost_fitness_vs_consumption = imputed_coreboost_data.groupby('Fitness Goal')['Average Weekly Consumption (Bottles)'].mean()
...
plt.figure(figsize=(10, 6))
coreboost_gym_vs_drink = imputed_coreboost_data.groupby('Preferred Sports Drink Type')['Hours at Gym (per week)'].mean()
...
plt.figure(figsize=(10, 6))
coreboost_gym_vs_consumption = imputed_coreboost_data.groupby('Hours at Gym (per week)')['Average Weekly Consumption (Bottles)'].mean()
...
plt.figure(figsize=(10, 6))
coreboost_fitness_vs_gym = imputed_coreboost_data.groupby('Fitness Goal')['Hours at Gym (per week)'].mean()
...
plt.figure(figsize=(10, 6))
coreboost_drink_vs_consumption = imputed_coreboost_data.groupby('Preferred Sports Drink Type')['Average Weekly Consumption (Bottles)'].mean()
...
plt.figure(figsize=(10, 6))
coreboost_drink_vs_fitness = pd.crosstab(imputed_coreboost_data['Preferred Sports Drink Type'], imputed_coreboost_data['Fitness Goal'])
